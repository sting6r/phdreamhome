...
const fetcher = async (u:string, signal?: AbortSignal)=>{
  try {
    ...
    if (e?.name === "AbortError") return undefined as any;
    throw e;
  }
};
const fetcherProfile = async (u:string, signal?: AbortSignal)=>{
  try {
    const r = await fetch(u, { signal, cache: "no-store" });
    if (!r.ok) return null;
...
function HomePageContent() {
  ...
  React.useEffect(() => {
    ...
    fetcher("/api/public-listings", sig).then((d) => {
      if (!alive) return; setListings(d?.listings ?? []); setIsLoading(false);
    }).catch(() => { if (alive) setIsLoading(false); });
    fetcherProfile("/api/public-profile", sig).then((p) => { if (alive) setProfile(p); });
              ...
              <div className="relative p-4 text-black">
                <div className="grid grid-cols-1 sm:grid-cols-[auto_1fr] gap-4 items-center sm:items-start">
                  <div className="w-40 h-40 sm:w-44 sm:h-44 rounded-full overflow-hidden bg-transparent relative mx-auto sm:mx-0">
                    {profile.imageUrl ? (
                      <Image src={profile.imageUrl} alt={profile.name || "Agent"} fill sizes="(min-width: 640px) 11rem, 10rem" className="object-cover" />
                    ) : (
              ...
              {presellingImages.length ? (
                <>
                  <Link prefetch={false} href="/properties/preselling" className="absolute inset-0 block">
                    <Image src={presellingImages[presellIndex]} alt="Preselling Property" fill className="object-cover" />
                  </Link>
            ...
            <div className="relative w-full h-40 sm:h-48 mb-3 rounded overflow-hidden">
              ...
              <div
                className={emphasizeStatus
                  ? "absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 text-sm px-4 py-2 rounded-full text-white border-2 border-white shadow-sm"
                  : "absolute top-2 left-2 text-xs px-3 py-1 rounded-full bg-[#DE6A4A] text-white border-2 border-white shadow-sm"}
