...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  let post = await prisma.blogPost.findUnique({ where: { id: idOrSlug }, include: { media: { orderBy: { sortOrder: "asc" } } } });
  if (!post) {
    const candidates = await prisma.blogPost.findMany({ where: { published: true }, include: { media: { orderBy: { sortOrder: "asc" } } } });
...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (typeof featured === "boolean" && featured === true) {
    await prisma.blogPost.updateMany({ where: { userId, id: { not: id } }, data: { featured: false } });
  }
  ...
  const updated = await prisma.blogPost.findUnique({ where: { id }, include: { media: { orderBy: { sortOrder: "asc" } } } });
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const userId = data.user?.id;
  if (!userId) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  const post = await prisma.blogPost.findUnique({ where: { id }, include: { media: true } });
