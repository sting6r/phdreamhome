  ...
  try {
    ...
    if (!buf) {
      ...
      if (token) {
        ...
        if (base) {
          const r = await fetch(`${base}/storage/v1/object/authenticated/${encodeURIComponent(bucketName)}/${encodeURIComponent(objectPath)}`, { headers: { Authorization: `Bearer ${token}` }, cache: "no-store" });
          if (r.ok) {
            buf = Buffer.from(await r.arrayBuffer());
            ct = r.headers.get("content-type");
          } else {
            // fallback to public if authenticated fails
            const r2 = await fetch(`${base}/storage/v1/object/public/${encodeURIComponent(bucketName)}/${encodeURIComponent(objectPath)}`, { cache: "no-store" });
            if (r2.ok) {
