...
export default function EditBlogPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  function applyTool(tool: "bold" | "italic" | "h1" | "h2" | "h3" | "list" | "link" | "separator") {
    ...
    if (tool === "bold") {
    ...
    } else if (tool === "h1" || tool === "h2" || tool === "h3") {
      const level = tool === "h1" ? "# " : tool === "h2" ? "## " : "### ";
      const lineStart = value.lastIndexOf("\n", start - 1) + 1;
      const lineEndIndex = value.indexOf("\n", end);
      const lineEnd = lineEndIndex === -1 ? value.length : lineEndIndex;
      const line = value.slice(lineStart, lineEnd).replace(/^#+\s*/, "");
      next = value.slice(0, lineStart) + level + line + value.slice(lineEnd);
      selStart = lineStart + level.length;
      ...
      if (start === end) {
        ...
        const segmentEnd =
          segmentEndIndex === -1 ? value.length : segmentEndIndex;
        ...
        const transformed = lines
          ...
          .join("\n");
        next =
          value.slice(0, segmentStart) + transformed + value.slice(segmentEnd);
  ...
  function applyLinkFromConfig() {
    ...
    const parenIndex = inserted.indexOf("(");
  ...
  useEffect(() => {
    ...
    function computeTopUnderStatus() {
      const wrapper = document.getElementById("status-links-wrapper");
      if (!wrapper) return null;
      const wrapperRect = wrapper.getBoundingClientRect();
      const aside = document.querySelector("aside .sticky") as HTMLElement | null;
      let top = wrapperRect.top + wrapperRect.height;
      if (aside) {
        const asideStyle = window.getComputedStyle(aside);
        const asideTop = parseFloat(asideStyle.top) || 0;
        top = Math.max(top, asideTop);
      }
      return top;
    }
    ...
    function handleScroll() {
      ...
      const scrollMax = Math.max(0, (doc.scrollHeight || 0) - window.innerHeight);
      const offset = window.scrollY || window.pageYOffset || 0;
      const progress = scrollMax > 0 ? offset / scrollMax : 0;

      // keep measurements up to date
      measureToolsBox();
      const topUnderStatus = computeTopUnderStatus();
      const toolsEl = document.getElementById("blog-tools-bar");
      if (!toolsEl || topUnderStatus === null) {
        // if we can't measure, fallback to previous behavior: only unlock on near-top
        if (toolsLockedRef.current && progress <= 0.1) {
      ...
      const toolsViewportTop = toolsRect.top;

      if (!toolsLockedRef.current) {
        // only lock when the toolbar has scrolled up to the status-links/aside level
        if (toolsViewportTop <= topUnderStatus && offset !== 0) {
          toolsLockedRef.current = true;
          setToolsLocked(true);
          setToolsTop(topUnderStatus);
        }
        return;
      }

      // when locked, only unfreeze if the user scrolls almost all the way back to top (90% up)
      if (progress <= 0.1) {
        toolsLockedRef.current = false;
        setToolsLocked(false);
        return;
      ...
      // keep toolbar aligned with status links / aside while locked
      setToolsTop(topUnderStatus);
    }
    ...
    function handleResize() {
      measureToolsBox();
      const topUnderStatus = computeTopUnderStatus();
      if (toolsLockedRef.current && topUnderStatus !== null) {
        setToolsTop(topUnderStatus);
      }
    }
    ...
    // initial measure
    measureToolsBox();
    const initialTop = computeTopUnderStatus();
    if (initialTop !== null && toolsLockedRef.current) setToolsTop(initialTop);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const parsedHeadings: string[] = [];
        const parsedSubHeadings: string[] = [];
        const parsedSubSubHeadings: string[] = [];

        const mediaItems: MediaItem[] = (p.media || []).map((m: any) => ({
          path: m.path,
          url: m.url || null,
          type: m.type,
        ...
        const newVideoAreas: { index: number; position: "above" | "below" }[] = [];
            ...
            for (const line of lines) {
              ...
              if (trimmed.startsWith("[image:") || trimmed.startsWith("[video:")) {
                ...
                if (match) {
                  ...
                  if (item) {
                    const pos = textStarted ? "below" : "above";
                    if (type === "image") {
                      newImageAreas.push({ index: idx, position: pos });
                      newImageUploads.push({ index: idx, position: pos, path: item.path, url: item.url });
                    } else {
                      newVideoAreas.push({ index: idx, position: pos });
                      newVideoUploads.push({ index: idx, position: pos, path: item.path, url: item.url });
                    }
                  }
                }
        ...
        parts.forEach((part: string, idx: number) => {
          ...
          for (const line of lines) {
            const trimmed = line.trim();
            if (!h3 && trimmed.startsWith("### ")) {
              h3 = trimmed.slice(4);
            } else if (!h2 && trimmed.startsWith("## ")) {
              h2 = trimmed.slice(3);
            } else if (!h1 && trimmed.startsWith("# ")) {
              h1 = trimmed.slice(2);
            } else if (trimmed.startsWith("[image:") || trimmed.startsWith("[video:")) {
              // skip placeholders in body lines
            } else {
              bodyLines.push(line);
            }
          }
          parsedTexts[idx] = bodyLines.join("\n");
          parsedHeadings[idx] = h1;
          parsedSubHeadings[idx] = h2;
          parsedSubSubHeadings[idx] = h3;
        });

        const main = parsedTexts[0] || "";
        const extras = parsedTexts.slice(1);
        const heroImage = mediaItems.find((m, i) => m.type === "image" && !usedMediaIndices.has(i)) || null;

        if (alive) {
          setTitle(p.title || "");
          setDescription(main);
          setPostedBy(p.author || "");
          setBlogOption(p.category || "Featured Projects");
          setDateText(p.displayDate || "");
          setExtraBlocks(extras);
          setHeadings(parsedHeadings);
          setSubHeadings(parsedSubHeadings);
          setSubSubHeadings(parsedSubSubHeadings);
          setPublished(!!p.published);
          setImageAreas(newImageAreas);
          setVideoAreas(newVideoAreas);
          setImageUploads(newImageUploads);
          setVideoUploads(newVideoUploads);
          setMedia(mediaItems);

          if (heroImage) {
            setHeroImageUpload({ path: heroImage.path, url: heroImage.url });
          } else {
  ...
  }, [showSubHeadingConfig, showSubSubHeadingConfig, showImageConfig, showVideoConfig, showLinkConfig, showIconMenu]);
    ...
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-4">
            <div className="mb-2">
              <div className="text-base font-semibold">Edit Blog</div>
              <div className="text-xs text-slate-600">
                Update your blog content and publication status.
              </div>
            </div>
            ...
            <div id="tools-container" className="w-full" style={toolsLocked ? { height: 42 } : undefined}>
              <div
                className="mb-3 z-20 relative bg-white w-full"
                style={
                  toolsLocked && toolsTop !== null && toolsBox
                    ? {
                        position: "fixed",
                        top: toolsTop,
                        left: toolsBox.left,
                    ...
                    {showIconMenu && (
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          setImageAreas(prev =>
                            existing ? prev : [...prev, { index: idx, position: pos }]
                          );
                      ...
                      <div className="mt-3 flex flex-col gap-2 text-xs border border-gray-300 bg-white p-3 rounded-md shadow-sm">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <div className="flex flex-col gap-1">
                            ...
                            <div className="text-[10px] text-slate-600">
                              Header will appear above:{" "}
                              {headingIndex === 0 ? "Main description" : `Block ${headingIndex}`}
                            </div>
                          </div>
                          <button
                            type="button"
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <div className="flex flex-col gap-1">
                            ...
                            <div className="text-[10px] text-slate-600">
                              Header and sub headings will appear above:{" "}
                              {subHeadingIndex === 0
                                ? "Main description"
                                : `Block ${subHeadingIndex}`}
                            </div>
                          </div>
                          <button
                            type="button"
                        ...
                        {showSubSubHeadingConfig && (
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <div className="flex flex-col gap-1">
                              ...
                              <div className="text-[10px] text-slate-600">
                                Sub sub heading will appear above:{" "}
                                {subSubHeadingIndex === 0
                                  ? "Main description"
                                  : `Block ${subSubHeadingIndex}`}
                              </div>
                            </div>
                            <button
                              type="button"
                      ...
                      <div className="mt-3 flex flex-col gap-2 text-xs border border-gray-300 bg-white p-3 rounded-md shadow-sm">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <div className="flex flex-col gap-1">
                              ...
                              onChange={e => {
                                ...
                                setImageAreas(prev => {
                                  const base =
                                    fromIdx === null
                                      ? prev
                                      : prev.filter(a => a.index !== fromIdx);
                                  const existing = base.find(a => a.index === nextIdx);
                                  if (existing) return base;
                                  return [
                                    ...base,
                                    { index: nextIdx, position: imageConfigPosition }
                                  ];
                                });
                                setShowImageConfig(false);
                              }}
                                ...
                                if (imageConfigIndex !== null) {
                                  setImageAreas(prev => {
                                    const filtered = prev.filter(
                                      a => a.index !== imageConfigIndex
                                    );
                                    return [
                                      ...filtered,
                                      { index: imageConfigIndex, position: pos }
                                    ];
                                  });
                                }
                              }}
                            ...
                            <div className="text-[10px] text-slate-600">
                              Image will appear {imageConfigPosition}{" "}
                              {imageConfigIndex === 0
                                ? "main description"
                                : `block ${imageConfigIndex}`}
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="mt-3 flex flex-col gap-2 text-xs border border-gray-300 bg-white p-3 rounded-md shadow-sm">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <div className="flex flex-col gap-1">
                              ...
                              onChange={e => {
                                ...
                                setVideoAreas(prev => {
                                  const base =
                                    fromIdx === null
                                      ? prev
                                      : prev.filter(v => v.index !== fromIdx);
                                  const existing = base.find(v => v.index === nextIdx);
                                  if (existing) return base;
                                  return [
                                    ...base,
                                    { index: nextIdx, position: videoConfigPosition }
                                  ];
                                });
                                setShowVideoConfig(false);
                              }}
                            ...
                            <select
                              ...
                              onChange={e => {
                                ...
                                if (videoConfigIndex !== null) {
                                  setVideoAreas(prev => {
                                    const filtered = prev.filter(
                                      v => v.index !== videoConfigIndex
                                    );
                                    return [
                                      ...filtered,
                                      { index: videoConfigIndex, position: pos }
                                    ];
                                  });
                                }
                                setShowVideoConfig(false);
                            ...
                            <div className="text-[10px] text-slate-600">
                              Video will appear {videoConfigPosition}{" "}
                              {videoConfigIndex === 0
                                ? "main description"
                                : `block ${videoConfigIndex}`}
                            </div>
                          </div>
                        </div>
                      </div>
              ...
              {uploadSuccess && (
                <div className="mt-2 mb-1 text-[10px] text-green-600">{uploadSuccess}</div>
              )}
            </div>
            </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative group">
                    <input
                      ...
                      value={dateText}
                      onChange={e => {
                        setDateText(formatDateInput(e.target.value));
                        setDateError(null);
                      }}
                      onBlur={e => {
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex items-start gap-2">
                  <textarea
                    ...
                    placeholder="Write a short description for this blog post"
                    value={description}
                    onChange={e => {
                      const val = e.target.value;
                      setDescription(prev => {
              ...
              <div className="mt-3 border-t pt-3">
                ...
                <div className="mt-2 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 space-y-4 text-xs">
                  ...
                  <div className="text-slate-600">
                    Posted by {postedBy || "Author name"} on{" "}
                  ...
                  {allBlocks.map((text, idx) => (
                    <div key={idx} className="text-slate-600">
                      ...
                      {headings[idx] && (
                        <div className="text-[12px] font-semibold text-slate-900 mb-0.5">
                          {headings[idx]}
                        </div>
                      )}
                      {subHeadings[idx] && (
                      ...
                      {subSubHeadings[idx] && (
                        <div className="text-[10px] font-medium text-slate-700 mb-0.5">
                          {subSubHeadings[idx]}
                        </div>
                      )}
                      {text
                ...
                {saveMsg && (
                  <div className="mt-2 text-[10px] text-green-600">{saveMsg}</div>
                )}
                {saveErr && (
                  <div
        ...
        <div className="fixed inset-0 z-[100] bg-black/70 flex items-center justify-center p-4 md:p-8 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[95vh] flex flex-col overflow-hidden shadow-2xl ring-1 ring-black/5">
                ...
                <header className="mb-10">
                  ...
                  <div className="flex items-center gap-4 text-slate-500 border-b border-slate-100 pb-8">
                    ...
                    <div>
                      ...
                      <div className="text-xs">{dateText || today} · 5 min read</div>
                    </div>
                  </div>
                </header>
            ...
            <div className="px-6 py-4 border-t border-slate-100 bg-slate-50 flex justify-end gap-3">
              ...
              >
                {published ? "Update Blog" : "Save Changes"}
              </button>
            </div>
          </div>
        </div>
