...
export default function ProfilePage() {
  ...
  async function sendProfileEmail() {
    setEmailErr(null); setEmailMsg(null); setSending(true);
    try {
      const r = await fetch("/api/profile/send-email", { method: "POST" });
      const j = await r.json().catch(()=>({}));
      if (!r.ok) { setEmailErr(j?.error || "Failed to send email"); return; }
  ...
  async function changePassword(e: React.FormEvent) {
    e.preventDefault(); setErr(null); setMsg(null); setPwErr(null); setPwMsg(null);
    try {
      if (!pw.next || pw.next.length < 8) throw new Error("Password must be at least 8 characters");
    ...
    } catch (e:any) { setPwErr(e.message || "Failed to change password"); }
  }
    ...
    <div className="space-y-6">
      <div className="card">
        ...
        <form onSubmit={save} className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div className="sm:col-span-2">
            ...
            <div className="mt-2">
              <button type="button" className="btn-outline" onClick={sendProfileEmail} disabled={sending || !form.email}>{sending ? "Sending..." : "Send Email"}</button>
              {emailErr && <div className="text-red-600 text-xs mt-1">{emailErr}</div>}
              {emailMsg && <div className="text-green-600 text-xs mt-1">{emailMsg}</div>}
            </div>
          </div>
          <div>
          ...
          <div className="sm:col-span-2"><button className="btn-blue btn-electric">Save Changes</button></div>
        </form>
      </div>
      ...
      <div className="text-xs text-black mb-3">Update your password here. Use a strong, unique password.</div>
        <form onSubmit={changePassword} className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div className="sm:col-span-2"><button className="btn-blue btn-electric">Change Password</button></div>
        </form>
      </div>
    </div>
  );
