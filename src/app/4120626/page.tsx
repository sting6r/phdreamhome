...
function LoginPageContent() {
  ...
  async function signInWithGoogle() {
    ...
    if (error) setErr("Google sign-in failed");
  ...
  }, [callbackUrl, isPreviewPing, router]);
  async function onSubmit(e: React.FormEvent) {
    e.preventDefault();
    setLoading(true);
    setErr(null);
    const { data, error } = await supabasePublic.auth.signInWithPassword({ email: identifier, password });
    setLoading(false);
    if (error || !data.session) { setErr("Invalid credentials"); return; }
