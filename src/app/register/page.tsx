...
export default function RegisterPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setLoading(false);
    if (error || !data.user) { setErr("Registration failed"); return; }
    try {
  ...
  async function resend() {
    ...
    try {
      ...
      if (!email) { setResendMsg("Enter email first"); return; }
      const { error } = await supabasePublic.auth.resend({ type: "signup", email });
      if (error) setResendMsg("Failed to send verification email");
      else setResendMsg(null);
    } finally {
      setResending(false);
    }
  }
        ...
        <form onSubmit={onSubmit} className="space-y-3">
          ...
          {msg && (
            <div className="space-y-2">
              ...
              {resendMsg && <div className="text-xs text-black">{resendMsg}</div>}
            </div>
          )}
