...
export default function Navbar() {
  ...
  async function loadProfileAvatar() {
    try {
      ...
      if (typeof document !== "undefined") {
        const hasToken = /(?:^|;\s*)sb-access-token=/.test(document.cookie || "");
        if (!hasToken) return;
      }
  ...
  useEffect(() => {
    ...
    supabasePublic.auth.getUser().then(async ({ data }) => {
      setName((data.user as any)?.user_metadata?.name || null);
      setEmail(data.user?.email || null);
      const mAvatar = (data.user as any)?.user_metadata?.avatar_url || (data.user as any)?.user_metadata?.picture || null;
      setAvatar(prev => prev || mAvatar || null);
      const u = data.user;
      if (u?.id && u.email && shouldSync(u.id)) {
        try {
          console.log("Attempting to sync user from Navbar (initial load).");
          await safePost("/api/auth/sync-user", { userId: u.id, email: u.email, name: (u as any)?.user_metadata?.name, username: (u as any)?.user_metadata?.user_name, phone: (u as any)?.phone || (u as any)?.user_metadata?.phone });
          console.log("Sync user request successful from Navbar (initial load).");
        } catch (syncError) {
          console.error("Sync user request failed from Navbar (initial load):", syncError);
        }
      }
    ...
    const { data: sub } = supabasePublic.auth.onAuthStateChange((_e, session) => {
      ...
      if (u?.id && u.email && shouldSync(u.id)) {
        try {
          console.log("Attempting to sync user from Navbar (auth state change).");
          safePost("/api/auth/sync-user", { userId: u.id, email: u.email, name: u?.user_metadata?.name, username: u?.user_metadata?.user_name, phone: u?.phone || u?.user_metadata?.phone });
          console.log("Sync user request successful from Navbar (auth state change).");
        } catch (syncError) {
          console.error("Sync user request failed from Navbar (auth state change):", syncError);
        }
      }
    ...
    return () => { sub.subscription.unsubscribe(); document.removeEventListener("mousedown", handle); };
  }, [open]);
            ...
            {mounted && loggedIn ? (
              <>
                <button aria-label="User menu" className="relative w-9 h-9 rounded-full bg-gray-200 overflow-hidden flex items-center justify-center" onClick={()=>setOpen(v=>!v)}>
                  {avatar ? <Image src={avatar} alt="avatar" fill sizes="36px" className="object-cover" /> : <span className="text-sm font-medium text-black">{initials}</span>}
                </button>
                {open && (
