...
function isVideo(url: string) {
  try { const u = new URL(url); return /\.(mp4|webm|ogg)$/i.test(u.pathname); } catch { return /\.(mp4|webm|ogg)$/i.test(url); }
}
      ...
      <div className="relative w-full h-72 sm:h-96 rounded-md overflow-hidden" onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
        {hasSrc && (isVideo(current.url) ? (
          <video
            ...
            onError={() => { try { const v = mainVideoEl.current; if (v) v.pause(); } catch {} }}
          />
        ) : (
          <Image src={current.url} alt="media" fill className="object-cover cursor-zoom-in" onClick={() => { setScale(1); setOpen(true); }} />
        ))}
        {!hasSrc && (
        ...
        <div className="mt-2 overflow-x-auto">
          <div className="flex gap-2">
              {items.map((it, i) => (
                ...
                {isVideo(it.url) ? (
                  <video src={it.url} className="absolute inset-0 w-full h-full object-cover" muted playsInline preload="none" controlsList="nodownload" />
                ) : (
                  <Image src={it.url} alt="thumb" fill sizes="96px" className="object-cover" />
                )}
                </button>
              ))}
          </div>
        ...
        <div className="fixed inset-0 bg-black/80 z-50">
          <div className="absolute inset-0 flex items-center justify-center" onWheel={(e)=>{ const d = e.deltaY < 0 ? 0.1 : -0.1; const ns = Math.min(3, Math.max(0.5, scale + d)); setScale(ns); }}>
            <div className="relative w-[90vw] h-[90vh]" style={{ transform: `scale(${scale})` }}>
              <Image src={current.url} alt="view" fill sizes="90vw" className="object-contain" />
            </div>
          </div>
