...
export default function StatusLinksCard() {
  ...
  function statusItem(label: string, slug: string, children?: React.ReactNode) {
    const isActive = pathname.startsWith(`/properties/${slug}`);
    return (
      <div
      ...
      >
        <button
          type="button"
          className={`text-base font-semibold leading-tight text-[#32004A] hover:text-blue-600 relative pb-1 after:content-[''] after:absolute after:bottom-0 after:left-0 after:w-full after:h-[3px] after:bg-purple-600 after:transition-transform after:duration-700 ${isActive ? "after:scale-x-100" : "after:scale-x-0 hover:after:scale-x-100"}`}
          aria-haspopup="menu"
        ...
        >
          {children || (
            <>
              <Link prefetch={false} role="menuitem" href={`/properties/${slug}`} className="block px-3 py-1.5 text-sm font-bold leading-normal text-blue-700 hover:bg-slate-100 border-b border-slate-100 mb-1">All {label}</Link>
  ...
  function blogItem() {
    ...
    const content = (
      <>
        <Link prefetch={false} role="menuitem" href="/blog" className="block px-3 py-1.5 text-sm font-bold leading-normal text-blue-700 hover:bg-slate-100 border-b border-slate-100 mb-1">All Blogs</Link>
        {categories.map(cat => (
          <Link 
            key={cat}
            prefetch={false} 
  ...
  function aboutItem() {
    ...
    const content = (
      <>
        <Link prefetch={false} role="menuitem" href="/about" className="block px-3 py-1.5 text-sm font-bold leading-normal text-blue-700 hover:bg-slate-100 border-b border-slate-100 mb-1">About PHDreamHome</Link>
  ...
  return (
    <div
      ref={containerRef}
      className={`relative border-b border-gray-200 ${mounted && scrolled ? "bg-[#F4DDFF]" : "bg-white"} w-full z-[100]`}
    >
      <div className="sm:hidden w-full bg-[#E5AFFF] border-b border-black/5 py-1.5 px-4 flex justify-between items-center">
        ...
        <Link 
             href="/contact" 
             prefetch={false} 
             className={`inline-flex items-center gap-2 px-4 py-2 rounded-full !bg-[#0EA5E9] !text-white hover:!bg-[#0EA5E9] transition-all shadow-md text-xs font-bold group ${!mounted ? "opacity-0 pointer-events-none" : ""}`}
            >
              <span className="!text-purple-900 group-hover:!text-white relative after:content-[''] after:absolute after:bottom-0 after:left-0 after:w-full after:h-[2px] after:bg-white after:transition-transform after:duration-700 after:scale-x-0 group-hover:after:scale-x-100 drop-shadow-[2px_2px_0px_rgba(0,0,0,0.2)] transition-all">
                ...
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" className="w-3.5 h-3.5 !text-purple-900 group-hover:!text-white drop-shadow-[2px_2px_0px_rgba(0,0,0,0.2)]"><path d="M21 15a2 2 0 0 1-2 2h-3l-4 4v-4H7a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v9z"/></svg>
            </Link>
      </div>
      <div className={`${mobileMenuOpen ? "flex" : "hidden"} sm:flex px-3 py-1.5 mx-auto flex-col sm:flex-row flex-wrap items-center justify-center sm:justify-center gap-x-6 gap-y-3 sm:gap-y-1.5 sm:gap-[2ch] text-center overflow-x-visible pb-4 sm:pb-1.5`}>
        <Link 
          prefetch={false} 
          href="/" 
          className={`text-base font-semibold leading-tight text-[#32004A] hover:text-blue-600 relative pb-1 after:content-[''] after:absolute after:bottom-0 after:left-0 after:w-full after:h-[3px] after:bg-purple-600 after:transition-transform after:duration-700 ${pathname === "/" ? "after:scale-x-100" : "after:scale-x-0 hover:after:scale-x-100"}`}
        >
          Home
        </Link>
        ...
        <Link 
          prefetch={false} 
          href="/contact" 
          className={`text-base font-semibold leading-tight text-[#32004A] hover:text-blue-600 relative pb-1 after:content-[''] after:absolute after:bottom-0 after:left-0 after:w-full after:h-[3px] after:bg-purple-600 after:transition-transform after:duration-700 ${pathname === "/contact" ? "after:scale-x-100" : "after:scale-x-0 hover:after:scale-x-100"}`}
        >
          Contact
        </Link>
      </div>
