...
export default function SimilarCarousel({ items }: { items: any[] }) {
  ...
  const perPage = 3;
  const pages = Array.from({ length: Math.max(1, Math.ceil((items?.length || 0) / perPage)) }, (_, i) => items.slice(i * perPage, i * perPage + perPage));
  ...
  if (!items || items.length === 0) return <div className="text-sm text-slate-700">No similar properties found.</div>;
  return (
                  ...
                  return (
                    <div key={l.id} className="card space-y-3 group transition-all duration-300 hover:shadow-[0_10px_30px_rgba(0,0,0,0.15)] hover:scale-[0.98] hover:ring-1 hover:ring-black/10">
                      <div className="relative w-full h-44 rounded overflow-hidden bg-slate-200">
                        {imgUrl ? <Image src={imgUrl} alt={l.title} fill className="object-cover" /> : null}
                        {l.status ? <div className="absolute top-2 left-2 text-xs px-3 py-1 rounded-full bg-[#DE6A4A] text-white border-2 border-white shadow-sm">{l.status}</div> : null}
