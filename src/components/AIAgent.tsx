...
export default function AIAgent() {
  ...
  const extractWebsiteContext = async () => {
    try {
      ...
      let listingsSnippet = "";
      try {
        const res = await fetch('/api/public-listings?featured=true', { cache: 'no-store' });
  ...
  useEffect(() => {
    ...
    const fetchProfileAndHistory = async () => {
      ...
      try {
        const res = await fetch("/api/profile", { signal: sig, cache: "no-store" });
        if (res.ok) {
                ...
                {!isFormSubmitted ? (
                  ...
                  <>
                    <div className="flex-1 overflow-y-auto p-4 space-y-2 bg-white">
                      ...
                      {(() => {
                        return chatSessions.slice().reverse().map((session, idx) => (
                          <div
                            key={`${session.id}-${idx}`}
